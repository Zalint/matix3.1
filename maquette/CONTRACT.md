...
## 5a. Comportements transverses posés par le moteur (lot de recette 1)

Ces mécanismes s'appliquent sans intervention de l'écran. Un écran peut s'en exclure ou les paramétrer par les attributs ci-dessous.

| Mécanisme | Ce que fait le moteur | Ce que l'écran peut faire |
|---|---|---|
| **Filtre et recherche** (R-03) | Pose une barre `.tfilt` au-dessus de chaque `.tbl-scroll` d'au moins 5 lignes de corps, hors tiroirs et modales : recherche plein texte, un menu par colonne filtrable, compte des lignes, réinitialisation. Une colonne est retenue si elle porte de 2 à 10 valeurs distinctes, courtes, sans montant ni bouton. | `<th data-nofilter>` exclut une colonne. La valeur d'une cellule est son `.st` ou son `.tag` s'il y en a un, sinon son texte principal débarrassé de `.hint`, `small`, `svg` et `.av`. |
| **Lignes écartées** | Le filtre écarte par la classe `f-out`, jamais par `hidden` : les écrans gardent `tr.hidden` pour leurs propres puces, une ligne s'affiche si aucun des deux ne l'écarte. | Une ligne `tr[data-empty]` est affichée quand le filtre ne laisse rien ; elle n'est jamais comptée dans les totaux. |
| **Tableau long** (R-02) | Ajoute `.tall` à tout `.tbl-scroll` dépassant 15 lignes visibles, ce qui plafonne sa hauteur et fige en-tête et pied. Recalculé à l'ouverture et après `ctx.filterRows`. | `ctx.refreshTall(el)` après un filtrage maison. |
| **Carte repliable** (R-09) | `data-fold="closed"` sur une `.card` : le moteur pose le bouton dans son `.ch` et masque tout ce qui suit, barre de filtre comprise. Redessine les graphiques au dépliage. | `data-fold-sum="…"` donne la ligne de résumé lue quand la carte est repliée. Texte brut, donc sans montant : le §2 interdit un montant en dur. |
| **Aveugle par périmètre** (R-16) | `data-blind="<périmètre>"` masque le contenu par la classe `blinded` pour les profils qui doivent déclarer ce périmètre, sans jamais le retirer du DOM : les scripts d'écran gardent leurs nœuds. Périmètres connus : `cg`, `scm`, `f4`, `f5`. | `data-blind-lbl="…"` remplace le libellé du cache. À poser sur un conteneur, jamais sur l'élément `[data-amt]` lui-même. **Attention** : l'écran Réconciliation utilise le même nom d'attribut avec des valeurs de profil (`caisse`, `dirops`, `collecteur`) pour son mécanisme local. Ne pas créer de périmètre homonyme d'un profil. |
| **Vue production** (R-04, R-05) | Le basculement de la barre du haut masque la pédagogie propre à la maquette : boutons et panneaux `help-*`, `.scr-head .scr-sub`, `.hub .hsub`. | `data-maquette-only` sur tout élément qui ne doit pas être livré. |
| **Séries de graphique** (R-08) | Dans un `.serpick`, chaque `[data-series-toggle]` reçoit le témoin de couleur et de trait de sa série. Aucune case cochée n'affiche aucune courbe. | Un `.multichart` sans aucune case dans l'écran retombe sur son `data-series`. |
